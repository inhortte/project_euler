...
abundants :: [Integer]
abundants = [a | a <- [1..28123], abundant a]

isNotSumOfAbundants :: Integer -> Bool
isNotSumOfAbundants n = all (\a -> a `notElem` abundants) abunDiffs
    where abunDiffs = takeWhile (>0) $ map (`subtract` n) abundants

ans = (sum [1..23]) + (sum $ filter isNotSumOfAbundants [24..28123])
